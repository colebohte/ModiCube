...
static void CheckSkin_Unchecked(struct Entity* e) {
	...
	if (Utils_IsFilePath(&skin)) {
		struct Stream stream;
		struct Bitmap bmp;
		cc_result res;

		res = Stream_OpenFile(&stream, &skin);
		if (res) { Logger_SimpleWarn2(res, "opening skin", &skin); return; }

		res = ApplySkin(e, &bmp, &stream, &skin);
		stream.Close(&stream);
		if (res) return;
		return;
	}
...
static void LocalPlayer_Init(struct LocalPlayer* p, int index) {
	struct HacksComp* hacks = &p->Hacks;
	cc_string skinPath; char skinPathBuffer[STRING_SIZE];
	...
	if (skinPath.length) {
		Entity_SetSkin(&p->Base, &skinPath);
	} else {

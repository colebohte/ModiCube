...
static struct MainScreen {
	...
	struct LButton btnRegister, btnOptions, btnSkin, btnUpdates;
	struct LInput iptUsername, iptPassword;
	struct LLabel lblStatus, lblSkin, lblUpdate;
	cc_bool signingIn;
} MainScreen CC_BIG_VAR;

#define MAINSCREEN_MAX_WIDGETS 14
...
static void MainScreen_UpdateSkinLabel(struct MainScreen* s) {
	cc_string skinPath, filename; char skinPathBuffer[STRING_SIZE], filenameBuffer[STRING_SIZE];
	cc_string text; char textBuffer[STRING_SIZE];

	String_InitArray(skinPath, skinPathBuffer);
	String_InitArray(filename, filenameBuffer);
	Options_Get(LOPT_SKIN_PATH, &skinPath, "");

	if (!skinPath.length) {
		LLabel_SetConst(&s->lblSkin, "&eSkin: default");
		return;
	}

	String_Copy(&filename, &skinPath);
	Utils_UNSAFE_GetFilename(&filename);

	String_InitArray(text, textBuffer);
	String_Format1(&text, "&eSkin: %s", &filename);
	LLabel_SetText(&s->lblSkin, &text);
}

static void MainScreen_SkinSelected(const cc_string* path) {
	Options_Set(LOPT_SKIN_PATH, path);
	Options_SaveIfChanged();
	MainScreen_UpdateSkinLabel(&MainScreen);
}

static void MainScreen_ChooseSkin(void* w) {
	static const char* const filters[] = { ".png", NULL };
	static struct OpenFileDialogArgs args = {
		"Skin files", filters,
		MainScreen_SkinSelected,
		0, NULL
	};
...
static void MainScreen_ApplyUpdateLabel(struct MainScreen* s) {
	...
	if (CheckUpdateTask.Base.success) {
		...
		LLabel_SetConst(&s->lblUpdate, latest > current ? "&aNew release (" GAME_APP_VER ")" : "&eUp to date (" GAME_APP_VER ")");
#else
		LLabel_SetConst(&s->lblUpdate, latest > current ? "&aNew release (" GAME_APP_VER ")" : "&eUp to date (" GAME_APP_VER ")");
#endif
	} else {
		LLabel_SetConst(&s->lblUpdate, "&cCheck failed (" GAME_APP_VER ")");
	}
}
...
static void MainScreen_Activated(struct LScreen* s_) {
	...
	LLabel_Add(s,  &s->lblSkin,    "&eSkin: default", main_lblSkin);
#ifdef CC_BUILD_NETWORKING
	...
	if (Platform_Flags & PLAT_FLAG_APP_EXIT) {
		LLabel_Add(s,  &s->lblUpdate,  "&eChecking.. (" GAME_APP_VER ")", main_lblUpdate_N);
		LButton_Add(s, &s->btnUpdates,  100, 35, "Exit", 
				MainScreen_ExitApp, main_btnUpdates);
	} else {
		LLabel_Add(s,  &s->lblUpdate,  "&eChecking.. (" GAME_APP_VER ")",

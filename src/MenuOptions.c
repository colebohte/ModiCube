...
static float Menu_Float(const cc_string *str) {
  float v;
  Convert_ParseFloat(str, &v);
  return v;
}

static void Menu_SwitchBindsClassic(void *a, void *b) {
  ClassicBindingsScreen_Show();
}
static void Menu_SwitchNostalgia(void *a, void *b) {
  NostalgiaMenuScreen_Show();
}
...
static struct MenuOptionsScreen {
  Screen_Body const char *descriptions[MENUOPTS_MAX_OPTS + 1];
  struct ButtonWidget *activeBtn;
  InitMenuOptions DoInit, DoRecreateExtra, OnHacksChanged,
      OnLightingModeServerChanged;
  int numButtons;
...
static void MenuOptionsScreen_Update(struct MenuOptionsScreen *s,
...
CC_NOINLINE static void
MenuOptionsScreen_FreeExtHelp(struct MenuOptionsScreen *s) {
  Elem_Free(&s->extHelp);
  s->extHelp.lines = 0;
}
...
static void MenuOptionsScreen_SelectExtHelp(struct MenuOptionsScreen *s,
                                            int idx) {
  const char *desc;
  cc_string descRaw, descLines[5];
  ...
  if (!s->widgets[idx])
    return;
  if (s->widgets[idx]->flags & WIDGET_FLAG_DISABLED)
    return;
...
static void MenuOptionsScreen_OnDone(const cc_string *value, cc_bool valid) {
  ...
  if (valid) {
    ...
    struct MenuOptionMetaBool *meta =
        (struct MenuOptionMetaBool *)btn->meta.ptr;
  ...
  if (s->selectedI >= 0)
    MenuOptionsScreen_SelectExtHelp(s, s->selectedI);
  s->activeBtn = NULL;
}
...
static int MenuOptionsScreen_PointerMove(void *screen, int id, int x, int y) {
  ...
  if (i == -1 || i == s->selectedI)
    return true;

  s->selectedI = i;
  if (!s->activeBtn)
    MenuOptionsScreen_SelectExtHelp(s, i);
  return true;
}
...
static int
MenuOptionsScreen_AddButton(struct MenuOptionsScreen *s, const char *name,
                            Widget_LeftClick onClick, Button_GetText getValue,
                            Button_SetText setValue, const char *desc) {
  struct ButtonWidget *btn;
  int i = s->numButtons;
...
static void MenuOptionsScreen_EndButtons(struct MenuOptionsScreen *s, int half,
                                         Widget_LeftClick backClick) {
  struct ButtonWidget *btn;
  int i, col, row, begRow;
  /* Auto calculate half/dividing count */
  if (half < 0)
    half = (s->numButtons + 1) / 2;
...
static void MenuOptionsScreen_InputClick(void *screen, void *widget) {
  ...
  MenuInputOverlay_Show(&meta->desc, &value, MenuOptionsScreen_OnDone,
                        Gui_TouchUI);
}
...
static void MenuOptionsScreen_HexSet(struct ButtonWidget *btn,
                                     const cc_string *v) {
  struct MenuOptionMetaHex *meta = (struct MenuOptionMetaHex *)btn->meta.ptr;
  cc_uint8 rgb[3];
...
void MenuOptionsScreen_AddHex(struct MenuOptionsScreen *s, const char *name,
                              PackedCol defaultValue, Button_GetHex getValue,
                              Button_SetHex setValue, const char *desc) {
  int i = MenuOptionsScreen_AddButton(s, name, MenuOptionsScreen_InputClick,
                                      MenuOptionsScreen_HexGet,
                                      MenuOptionsScreen_HexSet, desc);
...
static void MenuOptionsScreen_IntSet(struct ButtonWidget *btn,
                                     const cc_string *v) {
  struct MenuOptionMetaInt *meta = (struct MenuOptionMetaInt *)btn->meta.ptr;
  int value;
...
void MenuOptionsScreen_AddInt(struct MenuOptionsScreen *s, const char *name,
                              ...
                              const char *desc) {
  int i = MenuOptionsScreen_AddButton(s, name, MenuOptionsScreen_InputClick,
                                      MenuOptionsScreen_IntGet,
                                      MenuOptionsScreen_IntSet, desc);
...
static void MenuOptionsScreen_NumSet(struct ButtonWidget *btn,
                                     const cc_string *v) {
  struct MenuOptionMetaNum *meta = (struct MenuOptionMetaNum *)btn->meta.ptr;
  meta->SetValue(v);
}
...
void MenuOptionsScreen_AddNum(struct MenuOptionsScreen *s, const char *name,
                              ...
                              float defaultValue, Button_GetNum getValue,
                              Button_SetNum setValue, const char *desc) {
  int i = MenuOptionsScreen_AddButton(s, name, MenuOptionsScreen_InputClick,
                                      MenuOptionsScreen_NumGet,
                                      MenuOptionsScreen_NumSet, desc);
...
static void MenuOptionsScreen_OnLightingModeServerChanged(void *screen,
                                                          cc_uint8 oldMode,
                                                          cc_bool fromServer) {
  struct MenuOptionsScreen *s = (struct MenuOptionsScreen *)screen;
  /* This event only actually matters if it's from the server */
  if (fromServer) {
    if (s->OnLightingModeServerChanged)
      s->OnLightingModeServerChanged(s);
    s->dirty = true;
  }
}
...
static void MenuOptionsScreen_Init(void *screen) {
  ...
  TextGroupWidget_Create(&s->extHelp, 5, s->extHelpTextures,
                         MenuOptionsScreen_GetDesc);
  s->extHelp.lines = 0;
  Event_Register_(&UserEvents.HackPermsChanged, screen,
                  MenuOptionsScreen_OnHacksChanged);
  Event_Register_(&WorldEvents.LightingModeChanged, screen,
                  MenuOptionsScreen_OnLightingModeServerChanged);
...
static void MenuOptionsScreen_Render(void *screen, float delta) {
  ...
  Gfx_Draw2DFlat(w->x - EXTHELP_PAD, w->y - EXTHELP_PAD,
                 w->width + EXTHELP_PAD * 2, w->height + EXTHELP_PAD * 2,
                 tableColor);
...
static void MenuOptionsScreen_Free(void *screen) {
  Event_Unregister_(&UserEvents.HackPermsChanged, screen,
                    MenuOptionsScreen_OnHacksChanged);
  Event_Unregister_(&WorldEvents.LightingModeChanged, screen,
                    MenuOptionsScreen_OnLightingModeServerChanged);
  MenuInputOverlay_Close(false);
}
...
static void MenuOptionsScreen_ContextRecreated(void *screen) {
  ...
  for (i = 0; i < s->numButtons; i++) {
    if (s->widgets[i])
      MenuOptionsScreen_Update(s, &s->buttons[i]);
  }

  ButtonWidget_SetConst(&s->done, "Done", &s->titleFont);
  if (s->DoRecreateExtra)
    s->DoRecreateExtra(s);
  TextGroupWidget_SetFont(&s->extHelp, &s->textFont);
  TextGroupWidget_RedrawAll(
      &s->extHelp); /* TODO: SetFont should redrawall implicitly */
}

static const struct ScreenVTABLE MenuOptionsScreen_VTABLE = {
    MenuOptionsScreen_Init,
    Screen_NullUpdate,
    MenuOptionsScreen_Free,
    MenuOptionsScreen_Render,
    Screen_BuildMesh,
    Menu_InputDown,
    Screen_InputUp,
    Screen_TKeyPress,
    ...
    Menu_PointerDown,
    Screen_PointerUp,
    MenuOptionsScreen_PointerMove,
    Screen_TMouseScroll,
    MenuOptionsScreen_Layout,
    MenuOptionsScreen_ContextLost,
    MenuOptionsScreen_ContextRecreated};
...
static const char *const viewDistNames[VIEW_COUNT] = {"TINY", "SHORT", "NORMAL",
                                                      "FAR"};
...
static int ClO_GetViewDist(void) {
  if (Game_ViewDistance >= 512)
    return VIEW_FAR;
  if (Game_ViewDistance >= 128)
    return VIEW_NORMAL;
  if (Game_ViewDistance >= 32)
    return VIEW_SHORT;
...
static void ClO_SetViewDist(int v) {
  int dist = v == VIEW_FAR
                 ? 512
                 : (v == VIEW_NORMAL ? 128 : (v == VIEW_SHORT ? 32 : 8));
...
static void ClassicOptionsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  ...
  {
    MenuOptionsScreen_AddBool(s, "Music", ClO_GetMusic, ClO_SetMusic, NULL);
    MenuOptionsScreen_AddBool(s, "Invert mouse", ClO_GetInvert, ClO_SetInvert,
                              NULL);
    MenuOptionsScreen_AddEnum(s, "Render distance", viewDistNames, VIEW_COUNT,
                              ClO_GetViewDist, ClO_SetViewDist, NULL);
    MenuOptionsScreen_AddBool(s, "3d anaglyph", ClO_GetAnaglyph,
                              ClO_SetAnaglyph, NULL);

    MenuOptionsScreen_AddBool(s, "Sound", ClO_GetSounds, ClO_SetSounds, NULL);
    MenuOptionsScreen_AddBool(s, "Show FPS", ClO_GetShowFPS, ClO_SetShowFPS,
                              NULL);
    MenuOptionsScreen_AddBool(s, "View bobbing", ClO_GetViewBob, ClO_SetViewBob,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Limit framerate", ClO_GetFPS, ClO_SetFPS,
                              NULL);
    if (Game_ClassicHacks) {
      MenuOptionsScreen_AddBool(s, "Hacks enabled", ClO_GetHacks, ClO_SetHacks,
                                NULL);
    }
  }
...
static void ES_GetCloudsSpeed(cc_string *v) {
  String_AppendFloat(v, Env.CloudsSpeed, 2);
}
static void ES_SetCloudsSpeed(const cc_string *v) {
  Env_SetCloudsSpeed(Menu_Float(v));
}
...
static void ES_GetWeatherSpeed(cc_string *v) {
  String_AppendFloat(v, Env.WeatherSpeed, 2);
}
static void ES_SetWeatherSpeed(const cc_string *v) {
  Env_SetWeatherSpeed(Menu_Float(v));
}
...
static void EnvSettingsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    ...
    MenuOptionsScreen_AddNum(s, "Clouds speed", 0, 1000, 1, ES_GetCloudsSpeed,
                             ES_SetCloudsSpeed, NULL);
    MenuOptionsScreen_AddInt(s, "Clouds height", -10000, 10000,
                             World.Height + 2, ES_GetCloudsHeight,
                             ES_SetCloudsHeight, NULL);
    ...
    MenuOptionsScreen_AddEnum(s, "Weather", Weather_Names,
                              Array_Elems(Weather_Names), ES_GetWeather,
                              ES_SetWeather, NULL);
...
static void GraphicsOptionsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddEnum(
        s, "FPS mode", FpsLimit_Names, FPS_LIMIT_COUNT, GrO_GetFPS, GrO_SetFPS,
        "&eVSync: &fNumber of frames rendered is at most the monitor's refresh "
        "rate.\n"
        "&e30/60/120/144 FPS: &fRenders 30/60/120/144 frames at most each "
        "second.\n"
        "&eNoLimit: &fRenders as many frames as possible each second.\n"
        "&cNoLimit is pointless - it wastefully renders frames that you don't "
        "even see!");
    MenuOptionsScreen_AddInt(s, "View distance", 8, 4096, 512, GrO_GetViewDist,
                             GrO_SetViewDist, NULL);
    MenuOptionsScreen_AddBool(s, "Smooth lighting", GrO_GetSmooth,
                              GrO_SetSmooth,
                              "&eSmooth lighting smooths lighting and adds a "
                              "minor glow to bright blocks.\n"
                              "&cNote: &eThis setting may reduce performance.");
    MenuOptionsScreen_AddEnum(
        s, "Lighting mode", LightingMode_Names, LIGHTING_MODE_COUNT,
        ...
        "&cNote: &eIn multiplayer, this option may be changed or locked by the "
        "server.");

    MenuOptionsScreen_AddEnum(
        s, "Names", NameMode_Names, NAME_MODE_COUNT, GrO_GetNames, GrO_SetNames,
        ...
        "&eAllUnscaled: &fAll names of players are drawn see-through without "
        "scaling.");
    MenuOptionsScreen_AddEnum(
        s, "Shadows", ShadowMode_Names, SHADOW_MODE_COUNT, GrO_GetShadows,
        GrO_SetShadows,
        "&eNone: &fNo entity shadows are drawn.\n"
        "&eSnapToBlock: &fA square shadow is shown on block you are directly "
        "above.\n"
        "&eCircle: &fA circular shadow is shown across the blocks you are "
        "above.\n"
        "&eCircleAll: &fA circular shadow is shown underneath all entities.");

    if (!Gfx_GetUIOptions(s)) {
      MenuOptionsScreen_AddBool(s, "Mipmaps", GrO_GetMipmaps, GrO_SetMipmaps,
                                NULL);
    }
 ...
 *----------------------------------------------------ChatOptionsScreen-----------------------------------------------------*
 *#########################################################################################################################*/
static void ChatOptionsScreen_SetScale(const cc_string *v, float *target,
                                       const char *optKey) {
  *target = Menu_Float(v);
...
static void ChO_GetChatScale(cc_string *v) {
  String_AppendFloat(v, Gui.RawChatScale, 1);
}
...
static void ChatOptionsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddBool(s, "Scale with window", ChO_GetAutoScaleChat,
                              ChO_SetAutoScaleChat, NULL);
    MenuOptionsScreen_AddNum(s, "Chat scale", 0.25f, 4.00f, 1, ChO_GetChatScale,
                             ChO_SetChatScale, NULL);
    MenuOptionsScreen_AddInt(s, "Chat lines", 0, 30, Gui.DefaultLines,
                             ChO_GetChatlines, ChO_SetChatlines, NULL);

    MenuOptionsScreen_AddBool(s, "Log to disk", ChO_GetLogging, ChO_SetLogging,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Clickable chat", ChO_GetClickable,
                              ChO_SetClickable, NULL);
  }
  MenuOptionsScreen_EndButtons(s, -1, Menu_SwitchOptions);
}
...
static void GuO_GetHotbar(cc_string *v) {
  String_AppendFloat(v, Gui.RawHotbarScale, 1);
}
static void GuO_SetHotbar(const cc_string *v) {
  ChatOptionsScreen_SetScale(v, &Gui.RawHotbarScale, OPT_HOTBAR_SCALE);
}

static void GuO_GetInventory(cc_string *v) {
  String_AppendFloat(v, Gui.RawInventoryScale, 1);
}
static void GuO_SetInventory(const cc_string *v) {
  ChatOptionsScreen_SetScale(v, &Gui.RawInventoryScale, OPT_INVENTORY_SCALE);
}

static void GuO_GetCrosshair(cc_string *v) {
  String_AppendFloat(v, Gui.RawCrosshairScale, 1);
}
...
static void GuiOptionsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddBool(s, "Show FPS", GuO_GetShowFPS, GuO_SetShowFPS,
                              NULL);
    MenuOptionsScreen_AddNum(s, "Hotbar scale", 0.25f, 4.00f, 1, GuO_GetHotbar,
                             GuO_SetHotbar, NULL);
    MenuOptionsScreen_AddNum(s, "Inventory scale", 0.25f, 4.00f, 1,
                             GuO_GetInventory, GuO_SetInventory, NULL);
    MenuOptionsScreen_AddNum(s, "Crosshair scale", 0.25f, 4.00f, 1,
                             GuO_GetCrosshair, GuO_SetCrosshair, NULL);

    MenuOptionsScreen_AddBool(s, "Black text shadows", GuO_GetShadows,
                              GuO_SetShadows, NULL);
    MenuOptionsScreen_AddBool(s, "Tab auto-complete", GuO_GetTabAuto,
                              GuO_SetTabAuto, NULL);
    MenuOptionsScreen_AddBool(s, "Use system font", GuO_GetUseFont,
                              GuO_SetUseFont, NULL);
    MenuOptionsScreen_AddButton(s, "Select system font", Menu_SwitchFont, NULL,
                                NULL, NULL);
  }
  MenuOptionsScreen_EndButtons(s, -1, Menu_SwitchOptions);
}
...
static void HS_GetSpeed(cc_string *v) {
  String_AppendFloat(v, Entities.CurPlayer->Hacks.SpeedMultiplier, 2);
}
...
static cc_bool HS_GetWOMHacks(void) {
  return Entities.CurPlayer->Hacks.WOMStyleHacks;
}
...
static cc_bool HS_GetFullStep(void) {
  return Entities.CurPlayer->Hacks.FullBlockStep;
}
...
static cc_bool HS_GetPushback(void) {
  return Entities.CurPlayer->Hacks.PushbackPlacing;
}
...
static cc_bool HS_GetSlide(void) {
  return Entities.CurPlayer->Hacks.NoclipSlide;
}
...
static void HacksSettingsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddBool(s, "Hacks enabled", HS_GetHacks, HS_SetHacks,
                              NULL);
    MenuOptionsScreen_AddNum(s, "Speed multiplier", 0.1f, 50, 10, HS_GetSpeed,
                             HS_SetSpeed, NULL);
    MenuOptionsScreen_AddBool(
        s, "Camera clipping", HS_GetClipping, HS_SetClipping,
        "&eIf &fON&e, then the third person cameras will limit\n"
        "&etheir zoom distance if they hit a solid block.");
    MenuOptionsScreen_AddNum(s, "Jump height", 0.1f, 2048, 1.233f, HS_GetJump,
                             HS_SetJump,
                             "&eSets how many blocks high you can jump up.\n"
                             "&eNote: You jump much higher when holding down "
                             "the Speed key binding.");
    MenuOptionsScreen_AddBool(s, "WoM style hacks", HS_GetWOMHacks,
                              HS_SetWOMHacks,
                              "&eIf &fON&e, gives you a triple jump which "
                              "increases speed massively,\n"
                              "&ealong with older noclip style. This is based "
                              "on the \"World of Minecraft\"\n"
                              "&eclassic client mod, which popularized hacks "
                              "conventions and controls\n"
                              "&ebefore ClassiCube was created.");

    MenuOptionsScreen_AddBool(s, "Full block stepping", HS_GetFullStep,
                              HS_SetFullStep, NULL);
    MenuOptionsScreen_AddBool(s, "Breakable liquids", HS_GetLiquids,
                              HS_SetLiquids, NULL);
    MenuOptionsScreen_AddBool(
        s, "Pushback placing", HS_GetPushback, HS_SetPushback,
    ...
    MenuOptionsScreen_AddBool(
        s, "Noclip slide", HS_GetSlide, HS_SetSlide,
        "&eIf &fOFF&e, you will immediately stop when in noclip\n&emode and no "
        "movement keys are held down.");
    MenuOptionsScreen_AddInt(s, "Field of view", 1, 179, 70, HS_GetFOV,
                             HS_SetFOV, NULL);
  }
  MenuOptionsScreen_EndButtons(s, -1, Menu_SwitchOptions);
 ...
 *----------------------------------------------------MiscOptionsScreen----------------------------------------------------*
 *#########################################################################################################################*/
static void MiO_GetCameraMass(cc_string *v) {
  String_AppendFloat(v, Camera.Mass, 2);
}
...
static void MiO_GetReach(cc_string *v) {
  String_AppendFloat(v, Entities.CurPlayer->ReachDistance, 2);
}
static void MiO_SetReach(const cc_string *v) {
  Entities.CurPlayer->ReachDistance = Menu_Float(v);
}
...
static void MiscSettingsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddNum(s, "Reach distance", 1, 1024, 5, MiO_GetReach,
                             MiO_SetReach, NULL);
    MenuOptionsScreen_AddNum(s, "Camera Mass", 1, 100, 20, MiO_GetCameraMass,
                             MiO_SetCameraMass,
                             "&eChange the smoothness of the smooth camera.");
    MenuOptionsScreen_AddInt(s, "Music volume", 0, 100, DEFAULT_MUSIC_VOLUME,
                             MiO_GetMusic, MiO_SetMusic, NULL);
    MenuOptionsScreen_AddInt(s, "Sounds volume", 0, 100, DEFAULT_SOUNDS_VOLUME,
                             MiO_GetSounds, MiO_SetSounds, NULL);

    MenuOptionsScreen_AddBool(s, "Block physics", MiO_GetPhysics,
                              MiO_SetPhysics, NULL);
    MenuOptionsScreen_AddBool(s, "Smooth camera", MiO_GetCamera, MiO_SetCamera,
                              NULL);
    MenuOptionsScreen_AddBool(s, "View bobbing", MiO_GetViewBob, MiO_SetViewBob,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Invert mouse", MiO_GetInvert, MiO_SetInvert,
                              NULL);
...
static void NostalgiaAppearanceScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddBool(s, "Classic hand model", NA_GetHand, NA_SetHand,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Classic walk anim", NA_GetAnim, NA_SetAnim,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Classic chat", NA_GetClassicChat,
                              NA_SetClassicChat, NULL);
    MenuOptionsScreen_AddBool(s, "Classic inventory", NA_GetClassicInv,
                              NA_SetClassicInv, NULL);

    MenuOptionsScreen_AddBool(s, "Classic GUI textures", NA_GetGui, NA_SetGui,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Classic player list", NA_GetList, NA_SetList,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Classic options", NA_GetOpts, NA_SetOpts,
                              NULL);
  }
...
static void NostalgiaScreen_RecreateExtra(struct MenuOptionsScreen *s) {
  TextWidget_SetConst(&nostalgia_desc,
                      "&eRequires restarting game to take full effect",
                      &s->textFont);
}

static void
NostalgiaFunctionalityScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddBool(s, "Use server textures", NF_GetTexs, NF_SetTexs,
                              NULL);
    MenuOptionsScreen_AddBool(s, "Allow custom blocks", NF_GetCustom,
                              NF_SetCustom, NULL);

    MenuOptionsScreen_AddBool(s, "Non-classic features", NF_GetCPE, NF_SetCPE,
                              NULL);
    MenuOptionsScreen_AddButton(
        s, "Game version", NostalgiaScreen_Version, NF_GetVersion,
        NF_SetVersion,
...
/*########################################################################################################################*
 *------------------------------------------------ModiCubeOptionsScreen---------------------------------------------------*
 *#########################################################################################################################*/
static cc_bool MCO_GetCompass(void) { return Gui.ShowCompass; }
static void MCO_SetCompass(cc_bool v) {
  Gui.ShowCompass = v;
  Options_SetBool(OPT_SHOW_COMPASS, v);
}

static cc_bool MCO_GetServerName(void) { return Gui.ShowServerName; }
static void MCO_SetServerName(cc_bool v) {
  Gui.ShowServerName = v;
  Options_SetBool(OPT_SHOW_SERVER_NAME, v);
}

static cc_bool MCO_GetGameVer(void) { return Gui.ShowGameVersion; }
static void MCO_SetGameVer(cc_bool v) {
  Gui.ShowGameVersion = v;
  Options_SetBool(OPT_SHOW_GAME_VER, v);
}

static cc_bool MCO_GetMOTD(void) { return Gui.ShowMOTD; }
static void MCO_SetMOTD(cc_bool v) {
  Gui.ShowMOTD = v;
  Options_SetBool(OPT_SHOW_MOTD, v);
...
static void ModiCubeOptionsScreen_InitWidgets(struct MenuOptionsScreen *s) {
  MenuOptionsScreen_BeginButtons(s);
  {
    MenuOptionsScreen_AddBool(s, "Show compass", MCO_GetCompass, MCO_SetCompass,
                              "&eShow facing direction in F3 menu");
    MenuOptionsScreen_AddBool(s, "Show server name", MCO_GetServerName,
                              MCO_SetServerName,
...
void ModiCubeOptionsScreen_Show(void) {
  MenuOptionsScreen_Show(ModiCubeOptionsScreen_InitWidgets);
}

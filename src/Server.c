...
void Server_RetrieveTexturePack(const cc_string *url) {
  if (!Game_AllowServerTextures || TextureUrls_HasDenied(url))
    return;
...
int Ping_AveragePingMS(void) {
  ...
  /* (recv - send) is average time for packet to be sent to server and then sent
   * back. */
  /* However for ping, only want average time to send data to server, so half
   * the total. */
  totalMs /= 2;
...
static void SPConnection_BeginConnect(void) {
  ...
  if (SP_AutoloadMap.length) {
    Map_LoadFrom(&SP_AutoloadMap);
    return;
  }

#if defined CC_BUILD_NDS || defined CC_BUILD_PS1 || defined CC_BUILD_SATURN || \
    defined CC_BUILD_MACCLASSIC || defined CC_BUILD_TINYMEM
  horSize = 16;
  verSize = 16;
  ...
  verSize = 64;
    ...
    defined CC_BUILD_SATURN || defined CC_BUILD_TINYMEM
  gen = &FlatgrassGen;
#else
  gen = &NotchyGen;
#endif
...
static void SPConnection_SendBlock(int x, int y, int z, BlockID old,
                                   BlockID now) {
...
static void MPConnection_Fail(const cc_string *reason) {
  ...
  String_Format2(&msg, "Failed to connect to %s:%i", &Server.Address,
                 &Server.Port);
...
static void MPConnection_FailConnect(cc_result result) {
  static const cc_string reason = String_FromConst(
  ...
  if (result) {
    String_Format3(&msg, "Error connecting to %s:%i: %e" _NL, &Server.Address,
                   &Server.Port, &result);
    Logger_Log(&msg);
  }
...
static void MPConnection_BeginConnect(void) {
  static const cc_string invalid_reason =
      String_FromConst("Invalid IP address");
  cc_string title;
  ...
  if (Gui.ShowMOTD) {
    Gui.ShowMOTD = false;
    Options_SetBool(OPT_SHOW_MOTD, false);
    if (Entities.CurPlayer) {
      HacksComp_RecheckFlags(&Entities.CurPlayer->Hacks);
    }
    Chat_AddRaw("&eNote&f: Disable MOTD was turned off for your account's "
                "safety, re-enable in settings.");
  }

  /* Default block permissions (in case server supports SetBlockPermissions but
   * doesn't send) */
  Blocks.CanPlace[BLOCK_AIR] = false;
  Blocks.CanPlace[BLOCK_LAVA] = false;
  Blocks.CanDelete[BLOCK_LAVA] = false;
  Blocks.CanPlace[BLOCK_WATER] = false;
  Blocks.CanDelete[BLOCK_WATER] = false;
  Blocks.CanPlace[BLOCK_STILL_LAVA] = false;
  Blocks.CanDelete[BLOCK_STILL_LAVA] = false;
  Blocks.CanPlace[BLOCK_STILL_WATER] = false;
  Blocks.CanDelete[BLOCK_STILL_WATER] = false;
  Blocks.CanPlace[BLOCK_BEDROCK] = false;
  Blocks.CanDelete[BLOCK_BEDROCK] = false;

  res =
      Socket_ParseAddress(&Server.Address, Server.Port, addrs, &numValidAddrs);
  ...
  if (res && res != ReturnCode_SocketInProgess &&
      res != ReturnCode_SocketWouldBlock) {
    MPConnection_FailConnect(res);
  } else {
    Server.Disconnected = false;
    net_connecting = true;
    ...
    String_Format2(&title, "Connecting to %s:%i..", &Server.Address,
                   &Server.Port);
    LoadingScreen_Show(&title, &String_Empty);
  }
}

static void MPConnection_SendBlock(int x, int y, int z, BlockID old,
                                   BlockID now) {
  if (now == BLOCK_AIR) {
...
static void MPConnection_Disconnect(void) {
  ...
  static const cc_string reason =
      String_FromConst("You've lost connection to the server");
...
static void DisconnectReadFailed(cc_result res) {
  ...
  char msgBuffer[STRING_SIZE * 2];
  String_InitArray(msg, msgBuffer);
  String_Format3(&msg, "Error reading from %s:%i: %e" _NL, &Server.Address,
                 &Server.Port, &res);
...
static void DisconnectInvalidOpcode(cc_uint8 opcode) {
  ...
  String_Format2(&tmp, "Server sent invalid packet %b! (prev %b)", &opcode,
                 &lastOpcode);
...
static void MPConnection_Tick(struct ScheduledTask *task) {
  ...
  if (Server.Disconnected)
    return;
  if (net_connecting) {
    MPConnection_TickConnect(task);
    return;
  }

  /* NOTE: using a read call that is a multiple of 4096 (appears to?) improve
   * read performance */
  res = Socket_Read(net_socket, net_readCurrent, 4096 * 4, &read);

  if (res) {
    /* 'no data available for non-blocking read' is an expected error */
    if (res == ReturnCode_SocketInProgess)
      res = 0;
    if (res == ReturnCode_SocketWouldBlock)
      res = 0;

    if (res) {
      DisconnectReadFailed(res);
      return;
    }
  } else if (read == 0) {
    ...
    while (readCur < readEnd) {
      ...
      /* Workaround for older D3 servers which wrote one byte too many for
       * HackControl packets */
      if (cpe_needD3Fix && lastOpcode == OPCODE_HACK_CONTROL &&
      ...
      if (readCur + Protocol.Sizes[opcode] > readEnd)
        break;
      handler = Protocol.Handlers[opcode];
      if (!handler) {
        DisconnectInvalidOpcode(opcode);
        return;
      }
    ...
    /* These bytes are then later combined with subsequently read TCP packet
     * data */
  ...
  /* Network is ticked 60 times a second. We only send position updates 20 times
   * a second */
  if ((ticks++ % 3) != 0)
    return;
...
static void MPConnection_SendData(const cc_uint8 *data, cc_uint32 len) {
  ...
  while (len) {
    ...
    /* If sending would block (send buffer full), retry for a bit up to 10
     * seconds */
    /* TODO: Avoid doing this and manually buffer data when this happens */
    if (res && tries < 1000 &&
        (res == ReturnCode_SocketInProgess ||
         res == ReturnCode_SocketWouldBlock)) {
      Thread_Sleep(10);
      tries++;
      continue;
    ...
    /* NOTE: Not immediately disconnecting here, as otherwise we sometimes miss
     * out on kick messages */
    if (res) {
      net_writeFailure = res;
      return;
    }
    if (!wrote) {
      net_writeFailure = ERR_INVALID_ARGUMENT;
      return;
    }
